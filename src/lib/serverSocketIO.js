...
async function updateTransaksiJual(data) {
    try {
        ...
        const db = client.db('abadipos')
        //const collection = db.collection('dataTransaksijual')
        const tes = await db.collection('dataTransaksiJual').updateOne({ _id: data._id }, { $set: { pelanggan: data.pelanggan, jenis_order: data.jenis_order, totalTagihan: data.totalTagihan, totalDp: data.totalDp, item: data.item } })
        //update stok disini
